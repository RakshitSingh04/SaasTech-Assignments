      ...
      <ul>
        <li class="h3">&nbsp;${uiLabelMap.WorkEffortNotes}</li>
          <#--if project?has_content>
            <li><a href="<@ofbizUrl>newNotesForCompanyStore?workEffortId=${project.workEffortId!}&amp;showForm=Y</@ofbizUrl>">${uiLabelMap.CommonCreate}</a></li>
          <#else>
      ...
      <table width='100%' border='0' cellspacing='0' cellpadding='0' class='boxbottom'>
        <tr>
          <td>
                ...
                <tr>
                  ...
                  <td align="right" valign="top" width="15%">
                    <#if "N" == note.internalNote!>
                        <div>${uiLabelMap.CompanyStorePrintableNote}</div>
                          <#if project?has_content>
                            <form name="updateCompanyStoreNoteForPublicNote" method="post" action="<@ofbizUrl>updateCompanyStoreNote</@ofbizUrl>">
                    ...
                    <#if "Y" == note.internalNote!>
                        <div>${uiLabelMap.OrderNotPrintableNote}</div>
                           <#if project?has_content>
                             <form name="updateCompanyStoreNoteForPrivateNote" method="post" action="<@ofbizUrl>updateCompanyStoreNote</@ofbizUrl>">
                               <input type="hidden" name="workEffortId" value="${project.workEffortId!}" />
                               <input type="hidden" name="noteId" value="${note.noteId}" />
            ...
            <#else>
              <#if project?has_content>
                <div>&nbsp;${uiLabelMap.CompanyStoreCompanyStoreNoNotes}.</div>
              <#else>
                <div>&nbsp;${uiLabelMap.CompanyStoreTaskNoNotes}.</div>
              </#if>
      ...
      <#if parameters.showForm??>
        ...
        <div class="screenlet-body">
          <form name="createnoteform" method="post"
            <#if project?has_content> action="<@ofbizUrl>createNewNotesForCompanyStore</@ofbizUrl>"
