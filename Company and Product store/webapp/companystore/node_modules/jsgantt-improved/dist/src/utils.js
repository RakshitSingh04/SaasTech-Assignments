...
var getMinDate = function (pList, pFormat) {
    ...
    for (var i = 0; i < pList.length; i++) {
        if (!pList[i].hasStart())
            continue;
        if (!vDate || pList[i].getStart().getTime() < vDate.getTime())
            vDate = new Date(pList[i].getStart().getTime());
    }
    if (!vDate)
        vDate = new Date();
...
var getMaxDate = function (pList, pFormat) {
    ...
    for (var i = 0; i < pList.length; i++) {
        if (!pList[i].hasEnd())
            continue;
        if (!vDate || pList[i].getEnd().getTime() > vDate.getTime())
            vDate = new Date(pList[i].getEnd().getTime());
    }
    if (!vDate)
        vDate = new Date();
...
var formatDateStr = function (pDate, pDateFormatArr, pL) {
    ...
    for (var i = 0; i < pDateFormatArr.length; i++) {
        if (pDateFormatArr[i] !== null && typeof pDateFormatArr[i] === 'object' && 'lit' in pDateFormatArr[i]) {
            vDateStr += pDateFormatArr[i].lit;
            continue;
        }
...
var parseDateFormatStr = function (pFormatStr) {
    // {{token}} syntax: text inside {{}} is a date token; text outside is a literal.
    if (pFormatStr.indexOf('{{') !== -1) {
        var vDateFormatArray = new Array();
        var vRe = /\{\{([^}]+)\}\}|([^{]+|\{(?!\{))/g;
        var vMatch;
        while ((vMatch = vRe.exec(pFormatStr)) !== null) {
            if (vMatch[1] !== undefined) {
                vDateFormatArray.push(vMatch[1]); // date token, e.g. 'dd', 'mon'
            }
            else if (vMatch[2]) {
                vDateFormatArray.push({ lit: vMatch[2] }); // literal text
            }
        }
...
/**
 * Count working days between two dates (exclusive of pEnd).
 * Iterates day-by-day and increments only for days where pWorkingDays[day] is true.
 * Uses local dates to stay consistent with parseDateStr (which builds local-time dates).
 *
 * Example: countWorkingDays(Mon Jan 6, Mon Jan 13, {0:false,6:false}) === 5  (Mon–Fri only)
 */
var countWorkingDays = function (pStart, pEnd, pWorkingDays) {
    var count = 0;
    var cur = new Date(pStart.getFullYear(), pStart.getMonth(), pStart.getDate());
    var end = new Date(pEnd.getFullYear(), pEnd.getMonth(), pEnd.getDate());
    while (cur < end) {
        if (pWorkingDays[cur.getDay()])
            count++;
...
var hashKey = function (key) {
    return (0, exports.hashString)(key) % 10000;
};
