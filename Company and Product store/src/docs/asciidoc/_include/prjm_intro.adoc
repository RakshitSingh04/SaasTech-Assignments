...
under the License.
////
= CompanyStore Manager Introduction
...
CompanyStore views are created to support this.

== Status management
Status is managed at task level, CompanyStore and Phase statuses shown on screen are tasks status compilation. +
Task statuses are :
...
CompanyStore or Phase is completed if all included tasks are completed (or cancelled)
...
** CLIENT% : used to select parties which appears in CompanyStores Parties info, client side
** CLIENT_MANAGER : user login associated to a project with this roleType view it in ListCustomerCompanyStores
** CLIENT_ANALYST : same as CLIENT_MANAGER for ListCustomerCompanyStores
** CLIENT_BILLING : partyId which will be invoiced; same as CLIENT_MANAGER for ListCustomerCompanyStores; appears in Company part
   of CompanyStore Parties info, other CLIENT% role appears in contact part
** PROVIDER% : all parties which provide the project. In CompanyStores Parties info, provider part, all roleType not equals to CLIENT%
   will appear
