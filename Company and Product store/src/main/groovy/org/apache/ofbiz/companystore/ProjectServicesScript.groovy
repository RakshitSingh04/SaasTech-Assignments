 ...
 * under the License.
*/
package org.apache.ofbiz.companystore
 ...
 */
Map createCompanyStore() {
    Map serviceResult
    if (parameters.templateId) {
        serviceResult = run service: 'copyCompanyStore', with: [*: parameters,
    ...
    if (parameters.organizationPartyId) {
        run service: 'updateCompanyStoreRole', with: [*: serviceMap,
 ...
 */
Map updateCompanyStore() {
    run service: 'updateWorkEffort', with: parameters

    // Add roles
    if (parameters.organizationPartyId) {
        run service: 'updateCompanyStoreRole', with: [*: parameters,
                                                 partyId: parameters.organizationPartyId,
                                                 roleTypeId: 'INTERNAL_ORGANIZATIO']
    }
    if (parameters.clientBillingPartyId) {
...
/**
 * CompanyStore Scheduler sets the planning dates according task requirements and available resources
...
Map scheduleCompanyStore() {
    ...
    List tasks = from('CompanyStoreAndPhaseAndTask')
            .where(condition)
    ...
    while (!generalStartDate) {
        ...
        if (assocs) {
            assocs.each {
                BigDecimal hours = 0
                Map task = run service: 'getCompanyStoreTask', with: [taskId: it.workEffortIdFrom]
...
Map updateTimeEntryByWorkEffort() {
    ...
    if (!assigns) {
        Map serviceResult = run service: 'getCompanyStoreIdAndNameFromTask', with: [taskId: parameters.workEffortId]
 ...
 */
Map getCompanyStoreIdAndNameFromTask() {
    if (!parameters.taskId && !parameters.phaseId) {
        return failure(label('CompanyStoreUiLabels', 'CompanyStoreErrorCompanyStoreNotFound'))
    }
 ...
 * @return 'Success response containing the newCompanyStoreId in workEffortId and projectId
 */
Map copyCompanyStore() {
    GenericValue project = from('WorkEffort').where(workEffortId: parameters.projectId).queryOne()
    if (!project) {
        return error(label('CompanyStoreUiLabels', 'CompanyStoreErrorCompanyStoreNotFound'))
    }
    ...
    Map serviceResult = run service: 'createWorkEffort', with: [*: parameters,
                                                                workEffortId: null]
    String newCompanyStoreId = serviceResult.workEffortId
    ...
    from('WorkEffortAndPartyAssign')
            ...
            .each {
                run service: 'createWorkEffortPartyAssignment', with: [*: it.getAllFields(),
                                                                       workEffortId: newCompanyStoreId,
                                                                       fromDate: null,
                                                                       statusId: 'PAS_ASSIGNED']
            }
    ...
    project.getRelated('ChildWorkEffort', null, null, false).each {
        ...
        serviceResult = run service: 'createWorkEffort', with: [*: parameters,
                                                                ...
                                                                workEffortParentId: newCompanyStoreId,
    ...
    return success([workEffortId: newCompanyStoreId, projectId: newCompanyStoreId])
}

/**
 * get CompanyStore information
...
Map getCompanyStore() {
    ...
    Map highInfo = [projectId: project.workEffortId,
                    ...
                    parentCompanyStoreId: project.workEffortParentId]
...
/**
 * get CompanyStore Phase information
 * @return 'Success response containing the phaseList and projectId
 */
Map getCompanyStorePhaseList() {
    GenericValue project = from('WorkEffort').where(workEffortId: parameters.projectId).cache().queryOne()
    if (!project) {
        return error(label('CompanyStoreUiLabels', 'CompanyStoreErrorCompanyStoreNotFound'))
    }
...
/**
 * get CompanyStore Phase/task information
 * @return 'Success response containing the taskList and projectId
 */
Map getCompanyStoreTaskList() {
    GenericValue project = from('WorkEffort').where(workEffortId: parameters.projectId).cache().queryOne()
    if (!project) {
        return error(label('CompanyStoreUiLabels', 'CompanyStoreErrorCompanyStoreNotFound'))
    }
    ...
    List taskList = []
    from('CompanyStoreAndPhaseAndTask')
            .where(workEffortTypeId: 'PHASE', workEffortParentId: parameters.projectId)
...
/**
 * get CompanyStore information by party member
 * @return 'Success response containing the projectParties
 */
Map getCompanyStoresByParties() {
    EntityCondition condition = new EntityConditionBuilder().AND {
    ...
    List tasks = from('CompanyStoreAndPhaseAndTaskParty')
            .where(condition)
...
Map createTimeEntryInTimesheet() {
    ...
    if (parameters.fromDate && !parameters.timesheetId) {
        ...
        if (timesheet) {
            if (timesheet.statusId != 'TIMESHEET_IN_PROCESS') {
                return error(label('CompanyStoreUiLabels', 'CompanyStoreCannotAddToTimesheet'))
            }
    ...
    if (parameters.roleTypeId) {
        GenericValue taskRole = from('CompanyStorePartyAndPhaseAndTask')
 ...
 */
Map addCompanyStoreTimeToInvoice() {
    ...
    List tasks = from('CompanyStorePhaseTaskAndTimeEntryTimeSheet')
            .where(condition)
            .orderBy('workEffortId')
            .queryList()
    if (!tasks) {
        return error(label('WorkEffortUiLabels', 'CompanyStoreNoTimeentryItemsFound'))
    }
...
private Map combineActualHours(Map highInfo, String partyId) {
    ...
    Map notRatedValue = from('CompanyStorePhaseTaskActualNotRatedHoursView')
            .where(condition)
     ...
     */
    Map ratedValue = from('CompanyStorePhaseTaskActualRatedHoursView')
            .where(condition)
    ...
    notRatedValue = from('CompanyStorePhaseTaskActualNotRatedHoursView')
            .where(condition)
            .select('totalOriginalHours')
            .queryFirst()
    ...
    if (notRatedValue.totalOriginalHours) {
        ...
        ratedValue = from('CompanyStorePhaseTaskActualRatedHoursView')
                .where(condition)
                .select('totalOriginalHours', 'totalRatedHours')
                .queryFirst()
...
private String combineStatusInfo(Map highInfo, Map lowInfo = null) {
    ...
    long tasksCount = from('CompanyStorePhaseTaskAssignmentView').where(condition).queryCount()
    EntityCondition completedCond = new EntityConditionBuilder().AND {
        IN(taskStatusId: ['PTS_CANCELLED', 'PTS_COMPLETED'])
    }
    long completedTasks = from('CompanyStorePhaseTaskAssignmentView').where(condition).having(completedCond).queryCount()
    ...
    long assignedTasks = from('CompanyStorePhaseTaskAssignmentView').where(condition).having(assignedCond).queryCount()
    EntityCondition relatedCond = new EntityConditionBuilder().AND {
        GREATER_THAN_EQUAL_TO(entriesCount: 1L)
        GREATER_THAN_EQUAL_TO(resourceCount: 1L)
        NOT_IN(taskStatusId: ['PTS_CANCELLED', 'PTS_COMPLETED'])
    }
    long progressTasks = from('CompanyStorePhaseTaskAssignmentView').where(condition).having(relatedCond).queryCount()
...
private Map combineDatesAndPlannedHoursInfo(Map highInfo) {
    ...
    Map summaryInfo = from('CompanyStorePhaseTaskSklSumView')
            .where(condition)
    ...
    Map timeEntriesInfo = from('CompanyStorePhaseTaskActualEntrySumView')
            .where(condition)
