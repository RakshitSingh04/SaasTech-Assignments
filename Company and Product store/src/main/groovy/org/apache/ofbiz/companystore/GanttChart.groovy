...
final String permissionCheckEntity = 'COMPANYSTORE'
...
Map<String, Object> result = runService('getCompanyStore', [projectId: projectId, userLogin: userLogin])
...
result = runService('getCompanyStorePhaseList', [userLogin: userLogin, projectId: projectId])
...
if (phases) {
    phases.each { phase ->
        ...
        if (tasks) {
            tasks.each { task ->
                Map<String, Object> resultTaskInfo = runService('getCompanyStoreTask', [userLogin: userLogin, taskId: task.workEffortId])
                ...
                if (security.hasEntityPermission(permissionCheckEntity, '_READ', session as HttpSession) ||
                    ...
                    taskInfo.url = '/companystore/control/taskView?workEffortId=' + task.workEffortId
                } else {
                    taskInfo.url = ''
                }
