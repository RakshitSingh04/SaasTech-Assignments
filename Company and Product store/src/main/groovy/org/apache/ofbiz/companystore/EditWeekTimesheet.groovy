...
uiLabelMap = UtilProperties.getResourceBundleMap("CompanyStoreUiLabels", locale)
...
if (timesheetId) {
    ...
    entryIterator.close()
    if (timesheet == null) {
        result = runService('createCompanyStoreTimesheet', ["userLogin" : parameters.userLogin, "partyId" : partyId])
...
void retrieveWorkEffortData() {
        ...
        if (entryWorkEffort) {
            ...
            result = runService('getCompanyStoreIdAndNameFromTask', ["userLogin" : parameters.userLogin,"taskId" : entryWorkEffort.workEffortId])
...
if (timeEntry) {
    ...
    entry.d5 = day5Total
    entry.d6 = day6Total
    entry.phaseName = uiLabelMap.CompanyStoreTotals
